 ...
 *
 * Dynamically traverses all registered reset controllers and their reset
 * signals, finding the reset_control for the given reset ID.
 *
    ...
    list_for_each_entry(rcdev, &reset_controller_list, list) {
        ...
        for (i = 0; i < rcdev->nr_resets; i++) {
            if (current_idx == reset_id) {
                ...
                if (IS_ERR(rstc)) {
                    pr_warn("Failed to get reset control for rcdev %s, id %d: "
                            "%ld\n",
