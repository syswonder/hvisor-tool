...
static int handle_clock_describe_rates(SCMIDev *dev, uint16_t token,
    ...
    // /* Prepare ioctl arguments */
    // struct hvisor_scmi_clock_args args;
    // struct clock_rates_info *rates_info = (struct clock_rates_info
    // *)&args.u.data; rates_info->clock_id = clock_id; rates_info->rate_index =
    // rate_index;

    // /* Call the common ioctl function */
    // int ret = hvisor_scmi_ioctl(HVISOR_SCMI_CLOCK_DESCRIBE_RATES, &args,
    // sizeof(args)); if (ret < 0) {
    //     return scmi_make_response(dev, token, resp_iov, SCMI_ERR_GENERIC);
    // }
    ...
    // size_t available_for_rates = resp_iov->iov_len - resp_hdr_size -
    // fixed_payload_size; uint32_t max_rates_by_buffer =
    // (uint32_t)(available_for_rates / rate_entry_size); uint32_t num_return =
    // (rates_info->num_rates < max_rates_by_buffer) ? rates_info->num_rates :
    // max_rates_by_buffer;
...
static int handle_clock_config_set(SCMIDev *dev, uint16_t token,
    ...
    // double elapsed = (end.tv_sec - start.tv_sec) * 1000000.0 + (end.tv_nsec -
    // start.tv_nsec) / 1000.0;

    if (ret < 0) {
        // log_error("CLOCK_CONFIG_SET: clock_id=%u, config=%u, ioctl failed in
        // %.2f us", clock_id, attributes, elapsed);
        return scmi_make_response(dev, token, resp_iov, SCMI_ERR_GENERIC);
    }

    // log_error("CLOCK_CONFIG_SET: clock_id=%u, config=%u, ioctl took %.2f us",
    // clock_id, attributes, elapsed);
