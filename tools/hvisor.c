...
static __u64 load_image_to_memory(const char *path, __u64 load_paddr) {
    ...
    if (virt_addr == MAP_FAILED) {
        perror("Error mapping memory"); // Mapping memory failed, output error
                                        // message
        exit(1);                        // Exit program
    }

    memmove(virt_addr, image_content,
            map_size); // Move image content to mapped memory

    free(image_content);         // Free image content memory
    munmap(virt_addr, map_size); // Unmap memory

    close(fd); // Close file descriptor

    return map_size; // Return map size
}
...
static int zone_start_from_json(const char *json_config_path,
    ...
    if (!zone_id_json || !cpus_json || !name_json || !memory_regions_json ||
        !kernel_filepath_json || !dtb_filepath_json ||
        !kernel_load_paddr_json || !dtb_load_paddr_json || !entry_point_json ||
        !interrupts_json || !ivc_configs_json) {
